...
## pip下載BeautifulSoup套件

剛才抓到了HTML原始碼，但並不是所有資料都是我們想要的，這時就需要一個工具幫我們篩選出我們要的資料。BeautifulSoup就是Python一個用來解析HTML原始碼的套件。

和剛才一樣，在小黑窗內執行 `pip install beautifulsoup4` 下載BeautifulSoup套件。

![](https://i.imgur.com/AAE4tFS.png)

載完後，在程式碼最上方引用BeautifulSoup套件，`from bs4 import BeautifulSoup`，就可以使用了(請隊輔檢查)。
...
* **Text**：被元素所包起來的文字。
* **Attribute**：元素所擁有的屬性，像是class、style等等。
